...
class Order:
    def __init__(self, action: list[str]):
        self.time = int(action[0])
        self.symbol = action[1]
        self.quantity = int(action[3]) 
        self.price = int(action[4])
        if action[2] == SELL: self.quantity *= -1
...
class Portfolio:
    def __init__(self, initialCash: int = 1000):
        self.cash = initialCash
        self.positions: dict[str, Position] = {}
    
    def trade(self, action: list[str]):
        order = Order(action)
        self.cash -= order.quantity * order.price
        pos = self.positions.setdefault(order.symbol, Position(order.symbol, 0))
    ...
    def report(self) -> list[list[str]]:
        ...
        return [[CASH_SYMBOL, str(self.cash)]] + positions
    
actions = [["1", "AAPL", "B", "10", "10"], ["3", "GOOG", "B", "20", "5"], ["10", "AAPL", "S", "5", "15"]]
account = Portfolio()
for action in actions:
    account.trade(action)
reported = account.report()
print(reported)
...
#     For example, if t‍‌‍‍‍‍‍‌‍‌‍‌‍‍‍‍‍‌he customer purchases 5 shares of AAPL for 75 each, then finally 5 shares of AAPLO for 125, but their shares of AAPL can no longer be used to cover the deficit (since they've become collateral for AAPLO). As a result, 2 shares of GOOG would be sold back (again at 25, 5 AAPL, 5 AAPLO, and 3 GOOG. Thus, with an input of
